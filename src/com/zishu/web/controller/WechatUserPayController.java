package com.zishu.web.controller;
...
import com.zishu.web.bean.GetBrandWCPayRequestData;
import com.zishu.web.bean.MatrixToImageWriter;
import com.zishu.web.bean.OrderQueryRequestData;
import com.zishu.web.bean.OrderQueryResponseData;
import com.zishu.web.bean.UnifiedOrderNotifyRequestData;
import com.zishu.web.bean.UnifiedOrderNotifyResponseData;
import com.zishu.web.bean.UnifiedOrderRequestData;
import com.zishu.web.bean.UnifiedOrderResponseData;
import com.zishu.web.config.Configuration;
import com.zishu.web.util.Util;
import com.zishu.web.util.WxPayUtil;
 ...
 * @author:Frankhbu
 */
public class WechatUserPayController {
	
	private static Logger logger = Logger.getLogger(WechatUserPayController.class);
...
//	@RequestMapping("order/wechatWxPayNativePlaceOrder.php")
	...
	public UnifiedOrderResponseData unifiedOrder(String tradeType,Object object){
		...
		//2、调用统一下单接口
		logger.debug("UnifiedOrderRequestData => " + JSONObject.fromObject(data).toString());
		UnifiedOrderResponseData responseData = WxPayUtil.unifiedOder(data);
		logger.debug("UnifiedOrderResponseData => " + JSONObject.fromObject(responseData).toString());
		//3、根据统一下单接口返回数据修改本地订单信息
		
		logger.debug("结束调用微信统一下单方法...");
		return responseData;
	}
	...
	public JSONObject getJSSDKPackage(UnifiedOrderResponseData responseData){
		logger.debug("开始使用统一下单接口返回数据生成JSSDK所需json...");
		...
		logger.debug("JSSDK所需json数据  => " + json.toString());
		logger.debug("结束使用统一下单接口返回数据生成JSSDK所需json...");
		return json;
	}
	...
	public void getQRCode(UnifiedOrderResponseData responseData,HttpServletResponse response) {
		logger.debug("开始根据统一下单返回结果生成二维码...");
		logger.debug("code_url => " + responseData.getCode_url());
		...
		logger.debug("结束根据统一下单返回结果生成二维码...");
	}
	/**
	 * 
	 * TODO:接收微信服务器の支付结果回调
...
//	@RequestMapping("order/wechatWxPayGetPayResult.htm")
	public void getPayResult(HttpServletRequest request,HttpServletResponse response){
		...
		logger.debug("UnifiedOrderNotifyRequestData => " + JSONObject.fromObject(payResult).toString());
		...
		logger.debug("UnifiedOrderNotifyResponseData => " + responseString);
		//4、返回处理结果
		PrintWriter out = null;
		try {
			out = response.getWriter();
...
//	@RequestMapping("order/wechatWxPayQueryOrder.php")
	public void queryOrder(HttpServletRequest request,HttpServletResponse response){
		...
		logger.debug("orderQueryRequestData => " + JSONObject.fromObject(orderQueryRequestData).toString());
		//2、调用查询订单接口
		OrderQueryResponseData orderQueryResponseData = WxPayUtil.queryOrder(orderQueryRequestData);
		logger.debug("orderQueryResponseData => " + JSONObject.fromObject(orderQueryResponseData).toString());
		//3、根据接口返回结果
		//判断returnCode
...
//			result.put("msg", orderQueryResponseData.getReturn_msg());
		...
		try {
			...
			logger.debug("orderQueryResult =>" + json.toString());
			out = response.getWriter();
			out.print(json.toString());
		} catch (IOException e) {
			e.printStackTrace();
